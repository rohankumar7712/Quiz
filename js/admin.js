    ...
    .forEach(k => {
      ...
      list.innerHTML += `
        <div class="border rounded p-3 mb-3">
          ...
          <label class="form-label">End Time:</label>
          <input type="datetime-local" id="end_${code}" class="form-control mb-2" value="${q.endAt}">
          <button class="btn btn-sm btn-success mb-1" onclick="updateQuizTime('${code}')">Update Time</button>
          <button class="btn btn-sm btn-danger" onclick="resetAttemptsFor('${code}')">Reset Attempts</button>
        </div>
      `;
    });
}
...
/* ---------- ADMIN RESET ATTEMPTS FEATURE ---------- */
function resetAttemptsFor(code) {
  const keys = Object.keys(localStorage).filter(k => k.startsWith(`attempt_${code}_`));
  keys.forEach(k => localStorage.removeItem(k));
  alert(`✅ Cleared ${keys.length} attempt(s) for quiz code ${code}`);
}
